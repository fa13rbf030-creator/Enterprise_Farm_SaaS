...
from procurement_service.api import (
    purchase_order_approvals as api,
)
...
def test_openapi_registers_purchase_order_approval_paths():
...
def test_submit_requires_authentication():
    ...
    response = TestClient(app).post(
        f"/purchase-orders/{po_id}/submit-for-approval",
...
def test_submit_requires_create_permission():
    ...
    response = TestClient(app).post(
        ...
        headers=headers(
            user_id=user_id,
            tenant_id=tenant_id,
            permissions=["procurement.approvals.read"],
        ),
        json=create_payload(
            purchase_order_id=po_id,
...
def test_submit_rejects_cross_tenant():
    ...
    response = TestClient(app).post(
        ...
        headers=headers(
            ...
            permissions=["procurement.approvals.create"],
        ),
        json=create_payload(
            purchase_order_id=po_id,
            requested_by=user_id,
...
def test_approve_rejects_actor_spoof():
    ...
    response = TestClient(app).post(
        (
            f"/purchase-orders/{uuid4()}"
            "/approval-steps/1/approve"
        ),
        headers=headers(
            user_id=user_id,
            tenant_id=tenant_id,
            permissions=["procurement.approvals.approve"],
        ),
        json=decision_payload(decided_by=uuid4()),
    )
...
def test_reject_rejects_actor_spoof():
    ...
    response = TestClient(app).post(
        ...
        headers=headers(
            user_id=user_id,
            tenant_id=tenant_id,
            permissions=["procurement.approvals.reject"],
        ),
        json=decision_payload(decided_by=uuid4()),
    )
    ...
    class FakeCoordinator:
        ...
        async def submit_for_approval(self, **kwargs):
            raise DuplicatePurchaseOrderApprovalError(
                "duplicate purchase order approval"
            )
